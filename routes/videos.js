...
router.post('/register', requireAuth, async (req, res) => {
  try {
    ...
    let parsedTags = [];
    if (tags) parsedTags = (Array.isArray(tags) ? tags : tags.split(',')).map(t => t.trim().toLowerCase()).filter(t => t).slice(0, 10);
...
router.post('/upload', requireAuth, upload.single('video'), async (req, res) => {
  try {
    ...
    const vid = uuidv4(), ext = req.file.originalname.split('.').pop() || 'mp4';
    ...
    const { data: video, error: de } = await supabaseAdmin.from('videos').insert({
      id: vid, creator_id: req.user.id, title: title.trim().slice(0, 150),
      description: description?.trim().slice(0, 2000) || null, video_url: ud?.publicUrl,
      storage_path: path, thumbnail_url: thumbnail_url || null, tags: ptags,
      status: 'published', views: 0, likes_count: 0, comments_count: 0, shares_count: 0,
      file_size: req.file.size, created_at: new Date().toISOString()
    }).select().single();
...
router.get('/', optionalAuth, async (req, res) => {
  try {
    ...
    let q = supabaseAdmin.from('videos')
      .select('id,title,description,video_url,thumbnail_url,tags,views,likes_count,comments_count,shares_count,created_at,creator:users!creator_id(id,username,avatar_url)')
      .eq('status', 'published').order('created_at', { ascending: false }).range(offset, offset + limit - 1);
...
router.get('/mine', requireAuth, async (req, res) => {
  try {
    const { data: videos, error } = await supabaseAdmin.from('videos')
...
router.get('/:id', optionalAuth, async (req, res) => {
  try {
    const { data: video, error } = await supabaseAdmin.from('videos')
      .select('id,title,description,video_url,thumbnail_url,tags,views,likes_count,comments_count,shares_count,created_at,creator:users!creator_id(id,username,avatar_url,bio)')
...
router.get('/:id/comments', async (req, res) => {
  try {
    const page = Math.max(1, parseInt(req.query.page || '1', 10));
    const limit = Math.min(50, parseInt(req.query.limit || '20', 10));
    const { data: comments, error } = await supabaseAdmin.from('comments')
      .select('id,content,created_at,user:users!user_id(id,username,avatar_url)')
      .eq('video_id', req.params.id).order('created_at', { ascending: false }).range((page - 1) * limit, page * limit - 1);
    if (error) return res.status(500).json({ success: false, message: 'Erreur chargement' });
